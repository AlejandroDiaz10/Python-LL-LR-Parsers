...
def parse(rules, input):
    ...
    print(
        "{:^{w}}|{:^{w}}".format(
            "Stack de tokens y reglas", "String de tokens", w=int(WIDTH / 2)
        )
    )
    ...
    while True:
        if st.top().isupper():
            ...
            print(
                "{:<{w}}|{:>{w}} {} -> {}".format(
                    str(st), str(input), st.top(), rule, w=int(WIDTH / 2)
                )
            )
            if rule == "":
            ...
            if st.top() == input[0]:
                ...
                if st.top() == "$" and input[0] == "$":
                    print(
                        "{:<{w}}|{:>{w}} Accept".format(
                            str(st), str(input), w=int(WIDTH / 2)
                        )
                    )
                    return
                ...
                print(
                    "{:<{w}}|{:>{w}} match: {}".format(
                        str(st), str(input), st.top(), w=int(WIDTH / 2)
                    )
                )
                st.pop()
                del input[0]
