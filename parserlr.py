...
def parse(rules, grammar, input):
    ...
    print(
        "{:^{w}}|{:^{w}}".format(
            "Stack de estados, reglas y tokens", "String de tokens", w=int(WIDTH / 2)
        )
    )
    print("{arg:-<{w}}|{arg:->{w}}".format(arg="", w=WIDTH / 2))
    print(
        "{:<{w}}|{:>{w}} Estado inicial".format(str(st), str(input), w=int(WIDTH / 2))
    )
    ...
    while True:
        ...
        if action[0] == "s":
            # shift
            print(
                "{:<{w}}|{:>{w}} shift, next is {}".format(
                    str(st), str(input), action[1], w=int(WIDTH / 2)
                )
            )
            ...
            print(
                "{:<{w}}|{:>{w}} reduce [{}->{}], next is {}".format(
                    str(st), str(input), rule[0], rule[1], action[1], w=int(WIDTH / 2)
                )
            )
            ...
            print(
                "{:<{w}}|{:>{w}} Accept".format(str(st), str(input), w=int(WIDTH / 2))
            )
            return
...
class GrammarHW(GrammarLR):
    def __init__(self):
        super().__init__("table_hw_lr.csv", "grammar_hw.csv")


if __name__ == "__main__":
    Grammar1().parse("x+y*x*y")
    GrammarHW().parse("((a),a,(a,a))")
